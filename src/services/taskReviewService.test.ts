...
describe('taskReviewService', () => {
	it('builds completion, delay, focus and trend metrics from active tasks', () => {
		const review = buildTaskReview({
			tasks: [
				makeTask({ id: 'done-today', title: '完成 A', status: 'done', priority: 'high', group: '工作', tags: ['项目'], dueStart: todayStart + 9 * 60 * 60 * 1000, updatedAt: todayStart + 10 * 60 * 60 * 1000 }),
				makeTask({ id: 'overdue', title: '延期 B', status: 'todo', priority: 'urgent', group: '工作', tags: ['项目', '风险'], dueStart: todayStart - day }),
				makeTask({ id: 'open', title: '待办 C', status: 'doing', priority: 'medium', group: '生活', tags: ['习惯'], dueStart: todayStart + 2 * day }),
		...
		expect(review.completionTrend.at(-1)).toEqual({ date: '07-08', count: 1 });
	});
});
