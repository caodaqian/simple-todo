...
describe('taskReviewService', () => {
	it('builds completion, delay, focus and trend metrics from active tasks', () => {
		const review = buildTaskReview({
			tasks: [
				makeTask({ id: 'done-today', title: '完成 A', status: 'done', priority: 'high', group: '工作', tags: ['项目'], dueStart: todayStart + 9 * 60 * 60 * 1000, completedAt: todayStart - 2 * day, updatedAt: todayStart + 10 * 60 * 60 * 1000 }),
